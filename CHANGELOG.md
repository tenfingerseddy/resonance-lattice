...
- **Subscription capture, no key**: the bundled `rlat-learn` skill runs the same capture in-session on the user's Claude subscription — same four gates, the user approves every fact, landing via the key-free `rlat capture-attribute`. The curation-loop skills (`rlat-gap-scan`, `rlat-contradictions`, `rlat-refresh-facts`, `rlat-curate`) now ship in the public repo alongside it — every LLM-driven loop has a free subscription path; the API key is for unattended automation.
