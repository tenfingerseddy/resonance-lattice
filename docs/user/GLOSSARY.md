...
**Store** — the authoritative content layer of `rlat`'s three-layer architecture. Three modes ship in v2.0: bundled / local / remote. Local archives reconcile via `rlat refresh` (manual) or `rlat watch` (live); remote archives via `rlat sync` (all three land on `store/incremental.py`). `rlat freshness` is the read-only drift check on remote archives — useful as a CI gate before running sync. Resolves `(source_file, char_offset, char_length)` to text + computes drift status. See [STORE.md](../internal/STORE.md).
...
**Watch** — `rlat watch [km.rlat] [--once] [--verbose]`. Live, silent, self-discovering refresh loop on top of the same incremental delta-apply pipeline as `rlat refresh`. Zero-arg invocation auto-discovers `*.rlat` files in cwd and watches every recorded source root. FS events debounce-trigger a per-archive refresh; per-archive `threading.Lock` serialises the atomic-write path. Local mode only (bundled is immutable; remote → `rlat sync`). `--once` is the synchronous one-shot for CI / pre-commit. Mental model: events are hints to reconcile, not the unit of correctness — `bucketise` against the live source tree is the source of truth. Requires `pip install rlat[watch]`.
