...
- **Capture is low-friction and consent-gated.** Explicit: `rlat capture-attribute km.rlat "..." --kind attribute|constraint|falsified` (or `rlat capture-env` to auto-read machine facts). Passive: `rlat memory install-hooks --mine` opts a workspace into a four-gate session miner that learns world facts as you work — and drops facts about a *person* by design. That privacy gate held at zero leaks in validation.
...
- **No LLM in the retrieval loop.** `rlat search`, `profile`, `compare`, `summary`, `refresh`, `watch`, and `skill-context` need no API key. After a one-time `rlat install-encoder`, local-mode build/search runs offline. An Anthropic key is needed only for the LLM-calling verbs — `rlat deep-search`, `rlat grow`, `rlat memory verify`, `rlat reverify`, `rlat probe`, and the opt-in capture hooks — see [API keys](https://tenfingerseddy.github.io/resonance-lattice/api-keys.html).
