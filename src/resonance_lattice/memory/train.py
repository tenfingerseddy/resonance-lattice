"""Train operator surface — claim-level mutations from `/rlat-train` flags.
...
CLI to manage individual claims after a training run or distil pass
mis-classified one.

Maps to §0.7's three operator flags:

  rlat memory train --bad-vote <claim_id> [--why "..."]
  rlat memory train --good-vote <claim_id>
  rlat memory train --corroborate <claim_id>
...
class TrainResult:
    """One operator action against a single claim."""
...
def _audit_log_path(store: ExperienceClaimStore) -> Path:
    return store.root / _AUDIT_FILENAME


def _append_audit(
    store: ExperienceClaimStore, claim_id: str, action: str, *, why: str | None,
) -> None:
    ...
    line = f"{ts}  action={action}  claim_id={claim_id}"
    if why:
        line += f"  why={why!r}"
...
def _find_claim(store: ExperienceClaimStore, claim_id: str) -> Claim:
    claim = store.read(claim_id)
    if claim is None:
        raise KeyError(f"claim_id {claim_id!r} not in memory")
    return claim
...
def bad_vote(
    *, store: ExperienceClaimStore, claim_id: str, why: str | None = None,
) -> TrainResult:
    """Mark `claim_id` as `is_bad: true`. §0.6 retrieval drops it at gate 1.

    Idempotent — bad-voting an already-bad claim is a no-op (returns a
    TrainResult with `before == after == True` and no audit-log entry).
    """
    existing = _find_claim(store, claim_id)
    if existing.facts.is_bad:
        return TrainResult(
            claim_id=claim_id, action="bad-vote",
            field_changed="is_bad", before=True, after=True, why=why,
        )
    store.write(evolve(existing, is_bad=True))
    _append_audit(store, claim_id, "bad-vote", why=why)
    return TrainResult(
        claim_id=claim_id, action="bad-vote",
...
def good_vote(*, store: ExperienceClaimStore, claim_id: str) -> TrainResult:
    """Reverse a bad-vote: `is_bad: false`. Idempotent on claims that
    weren't bad-voted.
    """
    existing = _find_claim(store, claim_id)
    if not existing.facts.is_bad:
        return TrainResult(
            claim_id=claim_id, action="good-vote",
            field_changed="is_bad", before=False, after=False,
        )
    store.write(evolve(existing, is_bad=False))
    _append_audit(store, claim_id, "good-vote", why=None)
    return TrainResult(
        claim_id=claim_id, action="good-vote",
...
def corroborate(*, store: ExperienceClaimStore, claim_id: str) -> TrainResult:
    """Bump `recurrence_count` by exactly 1 + update `last_corroborated_at`.

    Lets the operator manually push a claim over the §0.6 recurrence ≥ 3
    ...
    """
    existing = _find_claim(store, claim_id)
    before = existing.facts.recurrence_count
    after = before + 1
    store.write(evolve(
        existing,
    ...
    _append_audit(store, claim_id, "corroborate", why=None)
    return TrainResult(
        claim_id=claim_id, action="corroborate",
